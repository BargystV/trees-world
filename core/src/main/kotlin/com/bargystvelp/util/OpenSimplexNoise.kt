...
object OpenSimplexNoise {
    ...
    private val SEED_FLIP_3D = -0x52D547B2E96ED629L

    private const val ROOT2OVER2 = 0.7071067811865476
    private const val SKEW_2D = 0.366025403784439
    private val UNSKEW_2D = -0.21132486540518713

    private const val ROOT3OVER3 = 0.577350269189626
    private val FALLBACK_ROTATE3 = 2.0 / 3.0
    private val ROTATE3_ORTHOGONALIZER = UNSKEW_2D

    private const val SKEW_4D = 0.309016994374947f
    private val UNSKEW_4D = -0.138196601125011f

    private const val N_GRADS_2D_EXPONENT = 7
    private const val N_GRADS_3D_EXPONENT = 8
    private const val N_GRADS_4D_EXPONENT = 9
    private val N_GRADS_2D = 1 shl N_GRADS_2D_EXPONENT
    private val N_GRADS_3D = 1 shl N_GRADS_3D_EXPONENT
    private val N_GRADS_4D = 1 shl N_GRADS_4D_EXPONENT
    ...
    private val RSQUARED_2D = 2.0f / 3.0f
    private val RSQUARED_3D = 3.0f / 4.0f
    private val RSQUARED_4D = 4.0f / 5.0f
     ...
     */
    fun noise2(
        seed: Long = System.currentTimeMillis(),    // Фиксированное зерно генерации, чтобы карта была постоянной
        x: Double,                                  // Масштаб шума по X: меньший делитель = крупнее паттерн
        y: Double                                   // Масштаб шума по Y
    ): Float {
     ...
     */
    private val GRADIENTS_2D: FloatArray
    private val GRADIENTS_3D: FloatArray
    private val GRADIENTS_4D: FloatArray
    private val LOOKUP_4D_A: IntArray
    private val LOOKUP_4D_B: Array<LatticeVertex4D?>

    init {
        GRADIENTS_2D = FloatArray(N_GRADS_2D * 2)
        ...
        run {
            var i = 0
            var j = 0
            while (i < OpenSimplexNoise.GRADIENTS_2D.size) {
                if (j == grad2.size) j = 0
        ...
        run {
            var i = 0
            var j = 0
            while (i < OpenSimplexNoise.GRADIENTS_3D.size) {
                if (j == grad3.size) j = 0
                OpenSimplexNoise.GRADIENTS_3D[i] = grad3[j]
                i++
                j++
            }
        }
        ...
        run {
            var i = 0
            var j = 0
            while (i < OpenSimplexNoise.GRADIENTS_4D.size) {
                if (j == grad4.size) j = 0
                OpenSimplexNoise.GRADIENTS_4D[i] = grad4[j]
                i++
                j++
            }
        }

        val lookup4DVertexCodes = arrayOf<IntArray?>(
            intArrayOf(
                0x15,
                0x45,
                0x51,
        ...
        LOOKUP_4D_B = arrayOfNulls<LatticeVertex4D?>(nLatticeVerticesTotal)
        var i = 0
        var j = 0
        while (i < 256) {
